...
```bash
npx neram mcp list --json      # tools, resources, resource templates, prompts — no client needed
npx neram mcp serve --port 3030  # loopback-only Streamable HTTP for Inspector testing
npx neram mcp install cursor --write --merge  # opt-in write; print-only by default
```

`mcp serve` binds `127.0.0.1` only and caps request bodies. `mcp install` prints the client snippet without writing files unless `--write` is passed; it refuses to overwrite malformed or unreadable configs, and re-running without `--merge` will not clobber an existing `neram` entry.
